...
class FilesServiceTest {
  ...
  void setUp() {
    ...
    when(applicationConfig.get(
            DocsConnectorServiceConfig.ApplicationConfig.MAX_FILE_SIZE_MB_DOCUMENT))
  ...
  @Test
  @DisplayName(
      "openFile should return a URL containing docs editor path when node is valid and within size"
          + " limit")
  ...
  @Test
  @DisplayName(
      "openFile should throw FileSizeTooLargeException when node size exceeds document limit")
  ...
  @Test
  @DisplayName(
      "uploadTemplate should return a CreatedFile with the node id when FilesClient upload"
          + " succeeds")
  ...
  @Test
  @DisplayName(
      "openFile with spreadsheet MIME type exceeding 10 MB should throw FileSizeTooLargeException")
    ...
    Assertions.assertThatThrownBy(
            () ->
                filesService.openFile(
                    ...
                    Optional.empty()))
        .isInstanceOf(
            com.zextras.carbonio.docs_connector.exceptions.FileSizeTooLargeException.class)
        .satisfies(
            e ->
                Assertions.assertThat(
                        ((com.zextras.carbonio.docs_connector.exceptions.FileSizeTooLargeException)
                                e)
                            .getMaxSizeLimitInMB())
                    .isEqualTo(10L));
  }
  ...
  @Test
  @DisplayName(
      "openFile with presentation MIME type exceeding 100 MB should throw"
  ...
  @Test
  @DisplayName(
      "openFile when applicationConfig returns empty for max-file-size key should throw (no"
          + " hardcoded fallback)")
  ...
  @Test
  @DisplayName(
      "openFile with both version and offsetFromUtc should include version in wopi endpoint query"
          + " string")
  void givenVersionAndOffsetFromUtcOpenFileShouldIncludeVersionInWopiSrc()
    ...
    String url =
        filesService.openFile(
            REQUESTER_ID, Locale.ENGLISH, COOKIE, NODE_ID, Optional.of(3), Optional.of(60));
  ...
  @Test
  @DisplayName(
      "openFile when quotaChecker reports over-quota should add permission=readonly to URL")
  ...
  private static final UUID INSTANCE_ID = UUID.fromString("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee");
  private static final String WOPI_BASE = "http://127.78.0.12:20000/wopi/" + NODE_ID;

  @Test
  @DisplayName(
      "WOPISrc: Advanced, no version, no offset → ends with ?service_id=<id>, no trailing &")
  void wopiSrc_advancedNoVersionNoOffset_exactServiceId()
    ...
    Assertions.assertThat(wopiSrc).isEqualTo(WOPI_BASE + "?service_id=" + INSTANCE_ID);
  }

  @Test
  @DisplayName(
      "WOPISrc: Advanced, version present, no offset → ?version=V&service_id=<id>, no trailing & or"
          + " &&")
  void wopiSrc_advancedVersionNoOffset_exactVersionAndServiceId()
    ...
    String wopiSrc = extractDecodedWopiSrc(url);
    Assertions.assertThat(wopiSrc).isEqualTo(WOPI_BASE + "?version=3&service_id=" + INSTANCE_ID);
  }
  ...
  void wopiSrc_ceVersionNoInstance_exactVersion()
    ...
    String wopiSrc = extractDecodedWopiSrc(url);
    Assertions.assertThat(wopiSrc).isEqualTo(WOPI_BASE + "?version=2");
  }
  ...
  void wopiSrc_advancedVersionAndOffset_exactAllParams()
    ...
    String url =
        filesService.openFile(
            REQUESTER_ID, Locale.ENGLISH, COOKIE, NODE_ID, Optional.of(5), Optional.of(60));

    String wopiSrc = extractDecodedWopiSrc(url);
    Assertions.assertThat(wopiSrc)
        .isEqualTo(WOPI_BASE + "?version=5&service_id=" + INSTANCE_ID + "&offset_from_utc=60");
  }
  ...
  void wopiSrc_advancedNoVersionWithOffset_exactServiceIdAndOffset()
    ...
    String url =
        filesService.openFile(
            REQUESTER_ID, Locale.ENGLISH, COOKIE, NODE_ID, Optional.empty(), Optional.of(120));
  ...
  void wopiSrc_ceNoVersionWithOffset_exactOffset()
    ...
    String url =
        filesService.openFile(
            REQUESTER_ID, Locale.ENGLISH, COOKIE, NODE_ID, Optional.empty(), Optional.of(180));

    String wopiSrc = extractDecodedWopiSrc(url);
    Assertions.assertThat(wopiSrc).isEqualTo(WOPI_BASE + "?offset_from_utc=180");
  }
  ...
  void wopiSrc_ceVersionAndOffset_exactVersionAndOffset()
    ...
    String url =
        filesService.openFile(
            REQUESTER_ID, Locale.ENGLISH, COOKIE, NODE_ID, Optional.of(7), Optional.of(240));

    String wopiSrc = extractDecodedWopiSrc(url);
    Assertions.assertThat(wopiSrc).isEqualTo(WOPI_BASE + "?version=7&offset_from_utc=240");
  }

  @Test
  @DisplayName(
      "openFile when selectInstance returns a UUID should embed service_id in both WOPISrc and"
          + " cool.html URL")
  ...
  @Test
  @DisplayName(
      "openFile with version AND instance should produce version=V&service_id=ID (single &, correct"
          + " order) in WOPISrc")
